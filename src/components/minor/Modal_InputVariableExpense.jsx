...
/**
 * gives the Date Object a custom function to return JS-Dates as date string
...
export default function InputVariableExpenseModal( props ) {
  ...
  const [categoryAutoCompleteValidationErrorMessage, setCategoryAutoCompleteValidationErrorMessage] = React.useState('');
  ...
  // Selection
  const [storeAutoComplete, setStoreAutoComplete] = React.useState('');
  const [categoryAutoComplete, setCategoryAutoComplete] = React.useState('');
  const [indulgencesAutoCompleteArray, setIndulgencesAutoCompleteArray]= React.useState([])
  const [sensitivitiesString, setSensitivitiesString] = React.useState('');

  // Presets
  const [presetDataArray, setPresetDataArray] = React.useState(null);
  React.useEffect(() => {
  ...
  const handleCategoryAutoCompleteSelection = (event, newValue) => {
    if (newValue && !categoryAutoCompleteItemArray.includes(newValue)) {
      setCategoryAutoComplete(newValue.trim())
    } else {
      setCategoryAutoComplete(newValue)
    }
  }

  const handleIndulgenceAutoCompleteSelection = (event, newValue) => {
   ...
   * @param {} event
   */
  const handleModalValuePresets = (event) => {
    let indulgences;
    switch (event.currentTarget.id) {
      case res.MINOR_INPUT_VARIABLE_EXPENSE_MODAL_PRESET_ID_TODAY:
        setPurchaseDate(new Date().yyyy_mm_dd())
        break;
      ...
      >
        <Box sx={style}>
          ...
          <FormControl fullWidth sx={{ m: 1 }} variant="standard">
            <InputLabel sx={{letterSpacing:2,fontWeight:300,fontSize:16}} htmlFor="description">{res.MINOR_INPUT_VARIABLE_EXPENSE_MODAL_DESCRIPTION}</InputLabel>
            <Input
              id="description"
              value={description}
          ...
          <Stack sx={{mt:3.5}}>
            <Autocomplete
              disablePortal
              selectOnFocus
              onChange={handleCategoryAutoCompleteSelection}
              inputValue={categoryAutoComplete}
              options={categoryAutoCompleteItemArray}
              renderInput={(params) => <TextField {...params} label={res.MINOR_INPUT_VARIABLE_EXPENSE_MODAL_CATEGORY} sx={{ borderRadius:0 }}/>}
              sx={{ ml:0.8, width:'100%' }}
            />
            <Typography sx={{ 
          ...
          <Stack sx={{mt:2.5}}>
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} label={res.MINOR_INPUT_VARIABLE_EXPENSE_MODAL_STORE} sx={{ borderRadius:0 }}/>}
              sx={{ ml:0.8, width:'100%' }}
            />
            <Typography sx={{ 
              color: 'rgba(211,47,47,1.0)', 
          ...
          <FormControl fullWidth sx={{ marginX: 1, mt:2 }}>
            <FormControlLabel
              control={<Checkbox
                checked={containsIndulgence}
                onChange={() => setContainsIndulgence(!containsIndulgence)}/>}
              label={
              <Typography sx={{
                letterSpacing:2,
                fontWeight:300,
          ...
          <Stack sx={{mt:1.5}}>
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} label={res.MINOR_INPUT_VARIABLE_EXPENSE_MODAL_INPUT_INDULGENCE} sx={{ borderRadius:0 }}/>}
              sx={{ ml:0.8, width:'100%' }}
            />
            <Typography sx={{ 
              color: 'rgba(211,47,47,1.0)', 
